...
async function run() {
    ...
    for (const vp of viewports) {
      ...
      for (const route of routes) {
        ...
        const geometry = await page.evaluate(() => {
          ...
          all.forEach((el) => {
            if (el.scrollWidth > viewportWidth + 1) { // 1px rounding tolerance
          ...
          return {
            ...
            appShellWidth,
            workspaceWidth,
            hasOverflow: docWidth > viewportWidth + 1,
            wideElements
          };
        });
  ...
  if (violations.length === 0) {
    console.log('✅ ZERO HORIZONTAL OVERFLOW DETECTED ACROSS ALL ROUTES, LANGUAGES, AND VIEWPORTS!');
  } else {
    console.log(`❌ FOUND ${violations.length} HORIZONTAL GEOMETRY VIOLATIONS:\n`);
    for (const v of violations) {
      ...
      console.log(`  AppShell scrollWidth: ${v.appShellWidth}px | Workspace scrollWidth: ${v.workspaceWidth}px`);
      console.log('  Top Wide Elements:');
      for (const el of v.wideElements.slice(0, 8)) {
