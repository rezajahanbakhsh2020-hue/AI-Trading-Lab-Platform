...
export const MarketScreener: React.FC<MarketScreenerProps> = ({
  ...
  const filteredItems = filterScreenerItems(
    INITIAL_SCREENER_ITEMS,
    selectedCategory,
    searchQuery,
    signalFilter
            ...
            <tbody className="divide-y divide-slate-800/60">
              {filteredItems.map((item) => (
                ...
                >
                  ...
                  <td className="py-2.5 px-3 text-end font-mono font-medium">${item.price.toLocaleString()}</td>
                  <td
                    className={`py-2.5 px-3 text-end font-bold font-mono ${
                      item.change24hPercent >= 0 ? 'text-emerald-400' : 'text-rose-400'
                    }`}
                  >
                  ...
                  <td className="py-2.5 px-3 text-end text-slate-400 font-mono">
                    ${(item.volume24hUsd / 1e9).toFixed(2)}B
                  </td>
